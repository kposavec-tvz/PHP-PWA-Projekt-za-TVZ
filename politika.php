...
<body>
	...
	<div class="okvir">
		...
		<main>
			<?php                
			$querry='SELECT * FROM clanci WHERE vrsta="politika" AND arhiviran IS NULL ORDER BY id DESC';
			//$querry='SELECT * FROM clanci WHERE vrsta="politika" ORDER BY id DESC LIMIT 12';
			...
			echo "<article>";
				...
				echo "</br>".substr($row['sadrzaj'], 0,500);
				if(strlen($row['sadrzaj'])>499)echo"...";
				echo '<form action="article.php" method="GET"><button name="klik" type="submit" value="'.$row['id'].'">See more</button></form></br>';
			echo "</article>";
		}
